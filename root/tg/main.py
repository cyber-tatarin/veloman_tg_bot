...
async def start(message: types.Message):
    ...
    await message.answer('Добро пожаловать в программу лояльности Veloman😊 '
                         ...
                         'Хотим подарить Вам денежный бонус на карту или на телефон🎁🎊\n\n'
...
async def wanna_leave_review(callback_query: types.CallbackQuery):
    await callback_query.message.answer('С какого устройства Вы хотите оставить отзыв',
                                        reply_markup=keyboards.get_ikb_to_choose_device_type_to_leave_review())
    await callback_query.message.edit_reply_markup(reply_markup=None)
    await callback_query.answer()


@logger.catch
@dp.callback_query_handler(lambda c: c.data == 'get_instruction_for_desktop', state='*')
async def wanna_leave_review(callback_query: types.CallbackQuery):
...
async def get_review_screenshot(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer('Спасибо за отзыв! Ваши данные отправлены менеджеру📨 '
                             ...
                             'Если Вы оставили отзыв о другом нашем товаре, то нажмите кнопку под этим сообщением⬇️',
                             reply_markup=keyboards.get_ikb_to_proceed_after_leaving_review())
